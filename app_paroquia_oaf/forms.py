...
class Formpessoa_valor_branco(forms.ModelForm):
    ...
    class Meta:
        model = pessoa_valor_branco
        fields = ['nome', 'valor','logradouro','numero_residencial','tipo_de_regularidade',]
...
class Formreceita_ate_50(forms.Form):
    ...
    mes_referente = forms.DateField(label="Data", initial=datetime.date(datetime.now()),widget=forms.DateInput(format="%d/%m/%Y",attrs={"class":"form-control", "style":"width:40%;","readonly":"readonly"}))
